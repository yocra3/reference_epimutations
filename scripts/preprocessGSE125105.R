...
## Load phenotypes ####
gse125105.full <- getGEO("GSE125105")

pheno  <- data.frame(Sample_Name = gse125105.full[[1]]$geo_accession ,
                     Disease = gse125105.full[[1]]$`diagnosis:ch1`,
                     Sex =  gse125105.full[[1]]$`Sex:ch1`, 
                     Age = as.numeric(gse125105.full[[1]]$`age:ch1`))
rownames(pheno) <- pheno$Sample_Name



## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE125105/")
...
dir.create("results/preprocess/GSE125105/")
  
outPrefix <- "results/preprocess/GSE125105/GSE125105"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Sex", "Disease", "Age")
...
## Se ven dos clusteres en el PCA no explicados por otra variable