...
###  gzip -cd GSE51057_series_matrix.txt.gz | head -n 1000 > GSE51057_mini.txt
#### Add !series_matrix_table_end  as final line
gse51057.full <- getGEO(filename = "data/GSE51057_mini.txt", getGPL = FALSE)

pheno  <- data.frame(Sample_Name = gse51057.full$geo_accession ,
                     Disease = gse51057.full$`cancer type (icd-10):ch1`,
                     Sex =  gse51057.full$`gender:ch1`, 
                     Age = as.numeric(gse51057.full$`age:ch1`),
                     Age_Menarch = as.numeric(gse51057.full$`age at menarche:ch1`))
rownames(pheno) <- pheno$Sample_Name



## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE51057/")
...
samplesheet$Age_Menarch <- pheno[samplesheet$Sample_Name, "Age_Menarch"]
...
dir.create("results/preprocess/GSE51057/")
  
outPrefix <- "results/preprocess/GSE51057/GSE51057"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Sex", "Disease", "Age", "Age_Menarch")
