...
## Load phenotypes ####
gse163970.full <- getGEO("GSE163970")

pheno  <- data.frame(Sample_Name = gse163970.full[[1]]$geo_accession ,
                     Disease = gse163970.full[[1]]$`phenotype:ch1`,
                     mmed = as.numeric(gse163970.full[[1]]$`mmed:ch1`),
                     umed = as.numeric(gse163970.full[[1]]$`umed:ch1`),
                     Scan =  gse163970.full[[1]]$`scan_date:ch1`)
rownames(pheno) <- pheno$Sample_Name



## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE163970/")
samplesheet$Scan <- pheno[samplesheet$Sample_Name, "Scan"]
samplesheet$Disease <- pheno[samplesheet$Sample_Name, "Disease"]
samplesheet$mmed <- pheno[samplesheet$Sample_Name, "mmed"]
samplesheet$umed <- pheno[samplesheet$Sample_Name, "umed"]
...
dir.create("results/preprocess/GSE163970/")
  
outPrefix <- "results/preprocess/GSE163970/GSE163970"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Scan", "Disease", "umed", "mmed")
...
## Se ve efecto del scan date pero no clusters diferenciados