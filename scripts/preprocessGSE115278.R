...
## Load phenotypes ####
gse115278.full <- getGEO("GSE115278")

pheno  <- data.frame(Sample_Name = gse115278.full[[1]]$geo_accession ,
                     Study = gse115278.full[[1]]$`study:ch1`,
                     Sex =  gse115278.full[[1]]$`Sex:ch1`, 
                     Age = as.numeric(gse115278.full[[1]]$`age:ch1`), 
                     Waist = gse115278.full[[1]]$`waist circumference (cm):ch1`)
...
## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE115278/")
samplesheet$Sex <- pheno[samplesheet$Sample_Name, "Sex"]
samplesheet$Study <- pheno[samplesheet$Sample_Name, "Study"]
samplesheet$Age <- pheno[samplesheet$Sample_Name, "Age"]
samplesheet$Waist <- pheno[samplesheet$Sample_Name, "Waist"]
...
dir.create("results/preprocess/GSE115278/")
  
outPrefix <- "results/preprocess/GSE115278/GSE115278"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Sex", "Study", "Age", "Waist")
