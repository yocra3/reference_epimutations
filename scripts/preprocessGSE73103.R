...
###  gzip -cd GSE73103_series_matrix.txt.gz | head -n 1000 > GSE73103_mini.txt
#### Add !series_matrix_table_end  as final line
gse73103.full <- getGEO(filename = "data/GSE73103_mini.txt", getGPL = FALSE)

pheno  <- data.frame(Sample_Name = gse73103.full$geo_accession ,
                     Weight = gse73103.full$`weight category:ch1`,
                     Sex =  gse73103.full$`Sex:ch1`, 
                     Age = as.numeric(gse73103.full$`age:ch1`))
...
## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE73103/")
samplesheet$Sex <- pheno[samplesheet$Sample_Name, "Sex"]
samplesheet$Weight <- pheno[samplesheet$Sample_Name, "Weight"]
...
dir.create("results/preprocess/GSE73103/")
  
outPrefix <- "results/preprocess/GSE73103/GSE73103"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Sex", "Weight", "Age")
...
## Se ve bastante dispersión en el PC1 pero no clusters claros.