...
## Load phenotypes ####
gse197676.full <- getGEO("GSE197676")

pheno  <- data.frame(Sample_Name = gse197676.full[[1]]$geo_accession ,
                     sex = gse197676.full[[1]]$`gender:ch1`,
                     age = as.numeric(gse197676.full[[1]]$`age:ch1`))
rownames(pheno) <- pheno$Sample_Name



## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE197676/")
samplesheet$Sex <- ifelse(pheno[samplesheet$Sample_Name, "sex"] == "Male", "M", "F")
...
dir.create("results/preprocess/GSE197676/")
  
outPrefix <- "results/preprocess/GSE197676/GSE197676"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Age", "Sex")
