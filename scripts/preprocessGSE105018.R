...
## Load phenotypes ####
gse105018.full <- getGEO("GSE105018")

pheno  <- data.frame(Sample_Name = gse105018.full[[1]]$geo_accession ,
                     FamilyID = gse105018.full[[1]]$`familyid:ch1`,
                     Sex =  gse105018.full[[1]]$`gender:ch1`, 
                     TwinID = gse105018.full[[1]]$`twinid:ch1`, 
                     Zygosity = gse105018.full[[1]]$`zygosity:ch1`)
rownames(pheno) <- pheno$Sample_Name

## Create samplesheet
samplesheet <- meffil.create.samplesheet(path = "data/GSE105018/")
samplesheet$Sex <- pheno[samplesheet$Sample_Name, "Sex"]
samplesheet$FamilyID <- pheno[samplesheet$Sample_Name, "FamilyID"]
samplesheet$TwinID <- pheno[samplesheet$Sample_Name, "TwinID"]
samplesheet$Zygosity <- pheno[samplesheet$Sample_Name, "Zygosity"]
...
dir.create("results/preprocess/GSE105018/")
  
outPrefix <- "results/preprocess/GSE105018/GSE105018"
...
batch_var <- c("Slide", "sentrix_col",  "sentrix_row", "Sex", "FamilyID", "TwinID", "Zygosity")
